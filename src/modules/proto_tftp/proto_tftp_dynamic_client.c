...
/**
 * $Id$
 * @file proto_tftp_dynamic_client.c
 * @brief TFTP dynamic clients
...
extern fr_dict_autoload_t proto_tftp_dynamic_client_dict[];
fr_dict_autoload_t proto_tftp_dynamic_client_dict[] = {
...
extern fr_dict_attr_autoload_t proto_tftp_dynamic_client_dict_attr[];
fr_dict_attr_autoload_t proto_tftp_dynamic_client_dict_attr[] = {
...
static unlang_action_t CC_HINT(nonnull) mod_process(UNUSED rlm_rcode_t *p_result, UNUSED module_ctx_t const *mctx, UNUSED request_t *request)
{
#if 0
	rlm_rcode_t rcode;
	CONF_SECTION *unlang;
	...
	case REQUEST_INIT:
		RDEBUG("Received %s ID %i", fr_tftp_codes[request->packet->code], request->packet->id);
...
extern fr_app_worker_t proto_tftp_dynamic_client;
fr_app_worker_t proto_tftp_dynamic_client = {
	.magic		= RLM_MODULE_INIT,
	.name		= "tftp_dynamic_client",
