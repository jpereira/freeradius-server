...
static tftp_state_t *fr_ftp_state_new(proto_tftp_process_t *inst, REQUEST *request, char const *ftp_dir, char const *filename, uint16_t block_size, bool is_ascii);

static int _fr_tftp_state_destroy(tftp_state_t *session);

static tftp_state_t *fr_ftp_state_lookup(proto_tftp_process_t *inst, REQUEST *request);

static int fr_tftp_state_cmp(const void *one, const void *two);

static bool fr_tftp_read_block(REQUEST *request, tftp_state_t *session, uint16_t block_num);
...
static tftp_state_t *fr_ftp_state_new(proto_tftp_process_t *inst, REQUEST *request, char const *ftp_dir, char const *filename, uint16_t block_size, bool is_ascii)
{
	...
	if (!S_ISREG(stat_buff.st_mode)) {
		REDEBUG("The %s must be a regular file.", fullpath);
		return NULL;
	}

	/*
	 *	We track the 'Read-Request' filename/blocksize/mode by the client src_port/src_ipaddr.
	 */
...
static void _tftp_reply_error(REQUEST *request, int error_code, char const *msg)
{
	VALUE_PAIR	*vp;
...
static rlm_rcode_t mod_process(module_ctx_t const *mctx, REQUEST *request)
{
	...
	case REQUEST_RECV:
		...
		switch (opcode) {
			case FR_PACKET_TYPE_VALUE_READ_REQUEST:
			{
				const char *filename;
				bool is_ascii;
				uint16_t block_size = FR_TFTP_DEFAULT_BLOCK_SIZE;
				 ...
				 */
				session = fr_ftp_state_new(inst, request, tftp_dir, filename, block_size, is_ascii);
				if (!session) {
	...
	send_reply:
		...
		if (!request->parent) {
			/*
			 *	Keep the state around only for DATA (it is a ACK response)
			 */
			if (request->reply->code != FR_PACKET_TYPE_VALUE_DATA) {
...
static int mod_instantiate(UNUSED void *instance, UNUSED CONF_SECTION *process_app_cs)
{
	...
	inst->session_tree = global_session_tree;
