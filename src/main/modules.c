 ...
 */
static const char CC_HINT(nonnull) *_dl_get_pathname(module_t const *addr_module) {
#ifdef HAVE_DLADDR
	static module_t const *addr_cached = NULL;
	static Dl_info dl_info;

	rad_assert(addr_module != NULL);
	rad_assert(addr_module->name != NULL); /* is a expected kind of module? */

	if (addr_cached != addr_module) { /* need to update dl_info? */
		if (!dladdr(addr_module, &dl_info)) {
			ERROR("Failed to call dladdr() to module %s, error='%s'", addr_module->name, dlerror());
			return NULL;
		}
		addr_cached = addr_module; /* update the cached addr */
	}

	/* Don't need update, return the same */
	return dl_info.dli_fname;
#else
	(void)addr_module;
	return "[dladdr() is unsuported]";
#endif
}
...
static int check_module_magic(CONF_SECTION *cs, module_t const *module)
{
	if (MAGIC_PREFIX(module->magic) != MAGIC_PREFIX(RADIUSD_MAGIC_NUMBER)) {
		cf_log_err_cs(cs, "Failed loading module rlm_%s from file %s", module->name, _dl_get_pathname(module));
