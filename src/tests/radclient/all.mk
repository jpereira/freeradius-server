...
$(OUTPUT)/%: $(DIR)/% | $(TEST).radiusd_kill $(TEST).radiusd_start
	...
	$(Q)if ! $(TESTBIN)/radclient $(ARGV) -f $< -d src/tests/radclient/config -D share/ 127.0.0.1:$(PORT) $(TYPE) $(SECRET) 1> $(FOUND) 2>&1; then \
		if [ "$(IGNORE_ERROR)" != "1" ]; then                               \
			...
			echo "RADCLIENT: $(TESTBIN)/radclient $(ARGV) -C $(RADCLIENT_CLIENT_PORT) -f $< -xF -d src/tests/radclient/config -D share/ 127.0.0.1:$(PORT) $(TYPE) $(SECRET)"; \
			exit 1;                                                     \
		fi;                                                                 \
	fi
#
...
#
	$(Q)if [ -e "$(EXPECTED)" ] && ! cmp -s $(FOUND) $(EXPECTED); then  \
		...
		echo "RADIUSD:   $(RADIUSD_RUN)";                           \
		echo "RADCLIENT: $(TESTBIN)/radclient $(ARGV) -C $(RADCLIENT_CLIENT_PORT) -f $< -d src/tests/radclient/config -D share/ 127.0.0.1:$(PORT) $(TYPE) $(SECRET)"; \
